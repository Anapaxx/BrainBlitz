    ...
    export default defineComponent({
        ...
        data() {
            return {
                ...
                messaggio: "Nessun utente trovato..." as string,
            }
        },
        methods: {
                ...
                if(this.ricerca){
                    ...
                    .then(response => {
                        ...
                        if (this.risultati.length === 0) {
                                if (this.utente && inputPulito === this.utente.Username) {
                                    this.messaggio = "Questo sei tu!";
                                } else if (this.giaRichiesta.some(r => r.Username === inputPulito)) {
                                    this.messaggio = "Richiesta ancora non accettata";
                                } else if (this.amici.some(a => a.Username === inputPulito)) {
                                    this.messaggio = "Siete già amici!";
                                } else {
                                    this.messaggio = "Nessun utente trovato...";
                                }
                                this.cliccato = true;
                            } else {
                                this.cliccato = false;
...
<template>
    <div class="impostazioni container-fluid col-10">
        <div class="row riga text-center col-12">
            ...
            <div v-if="risultati.length === 0 && cliccato" class="mb-4 fs-4">{{ messaggio }}</div>
