    ...
    export default defineComponent({
        ...
        methods: {
            ...
            ImpostaBadge() {
                if (this.utente) {
                    if (this.scelta === 0) {
                        // Rimuove il badge equipaggiato
                        axios.patch("/api/badge/equipaggiato", {
                            Username: this.utente.Username,
                            Scelta: null, // Imposta il badge equipaggiato a null
                        }).then(() => {
                            this.scelta = null; // Aggiorna la scelta localmente
                        });
                    } else if (this.scelta) {
                        // Cambia il badge equipaggiato
                        axios.patch("/api/badge/equipaggiato", {
                            Username: this.utente.Username,
                            Scelta: this.scelta,
                        });
    ...
    <div class="d-flex container-fluid col-11 justify-content-center"> <!--per avere una spaziatura laterale fissa-->
        <div class="row riga d-flex justify-content-center col-12">
            <div class="col-12 text-center">
                <img v-if="scelta !== null && scelta !== undefined && scelta != 0" :src="'/img/badge/' + scelta + '.png'" :alt="'Badge ' + scelta" class="premio"/>            
            </div>  
            <div class="col-12 text-center">
            ...
            <div class="col-12">
                <div class="col-12 col-md-6 offset-md-3">
                    <select v-model="scelta" id="theme-options" name="theme-options" class="form-select w-100 w-lg-50 h-15 mx-auto">                    
                        <option v-if="badgeSbloccati.length != 0" :value="null" disable selected hidden>Aggiungi un cappellino al tuo avatar!</option>
                        ...
                        <option value= "0">Nessun Badge</option>
                    </select>
                </div>
                <table class="table mt-4 rounded-table">
                    ...
                    <tbody>
                        <tr v-for="badge in badges" :key="badge.IDBadge" :class="{ 'table-secondary': !badgeSbloccati.some(b => b.IDBadge === badge.IDBadge) }">
                            <!--per ogni badge, table-secondary viene applicata se nell'array BadgeSbloccati non sia un ID uguale a IDBadge-->
